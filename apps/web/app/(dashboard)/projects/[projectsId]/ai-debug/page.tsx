...
export default function AiDebugPage() {
  ...
  const filteredSessions = useMemo(() => {
    ...
    return sessions.filter((session) => {
      ...
      if (query) {
        const haystack = [session.title, session.language, session.projectId]
          .filter(Boolean)
          .join(" ")
          .toLowerCase();
  ...
  }, [sessions, search, status, language, projectId]);
    ...
    <div className="flex h-full flex-col gap-6">
      ...
      {!isLoading && !isError && !hasSessions && (
        <div className="flex flex-1 flex-col items-center justify-center rounded-xl border border-neutral-border bg-neutral-surface-1 p-8 text-center">
          ...
          <Link
            href="/ai-debug/new"
            className="mt-5 inline-flex items-center gap-2 rounded-lg bg-brand-primary px-4 py-2.5 text-sm font-medium text-white shadow-lg shadow-brand-primary/20 transition-all hover:bg-brand-primary/90 active:scale-95"
          >
            <Plus size={16} />
            New Session
          </Link>
      ...
      {!isLoading && !isError && hasSessions && (
        <div className="flex flex-col gap-6">
          ...
          {/* Activity + resolution/language/attention */}
          ...
          <div className="flex flex-col gap-4">
            ...
            <SessionSearchToolbar
              ...
              projectIds={analytics.projectIds}
            />
