...
export default function ApiKeysTable({
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-[#131221] overflow-hidden">
      <table className="w-full text-left text-sm text-slate-300">
        <thead className="bg-white/5 text-xs uppercase text-slate-500 border-b border-white/10">
          <tr>
        ...
        <tbody className="divide-y divide-white/5">
          {API_KEYS.map((key) => (
            <tr key={key.id} className="transition-colors hover:bg-white/[0.02]">
              ...
              <td className="px-6 py-4 text-right">
                <div className="flex items-center justify-end gap-2">
                  ...
                  <button
                    className="p-2 text-slate-400 hover:text-red-400 transition-colors rounded-md hover:bg-white/10"
                    title="Revoke"
                  >
